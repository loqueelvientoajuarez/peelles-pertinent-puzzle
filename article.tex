...
\begin{equation} 
   \vcov = \begin{pmatrix} 
     ...
     \reldev^2\data_1\data_2              & \absdev^2\cot^2 + \reldev^2\data_2^2
            \end{pmatrix},
\end{equation}
when the second-order error term ($\absdev\reldev$) is ignored.  Then, the least squares estimate for $\data$ 
\begin{equation}
    {\data} = \frac{\data_1 + \data_2}{2} 
            \left(1 + \reldev^2\frac{(\data_1-\data_2)^2}{2\tau^2\absdev^2}\right)^{-1}
\end{equation}
systematically falls below the average.  If measurements differ significantly, it can even fall below the lowest value.  Figure~\ref{fig:peelle} gives such an example with 
\begin{align*}
    \cot   &= 1.250 \pm 0.063\ (5\%),\\
    \raw_1 &= 0.634 \pm 0.004\ (0.6\%),\\
    \raw_2 &= 0.646 \pm 0.004\ (0.6\%),
\intertext{which yields two points 2.1 standard deviations appart}
    \data_1 &= \system{0.793}{0.005}{0.040},\\
    \data_2 &= \system{0.808}{0.005}{0.040}
\intertext{and the estimate}
    \data &= \system{0.791}{0.003}{0.040}
\end{align*}
falls outside the data range.
\begin{figure}
\includegraphics[width=\linewidth]{pdf/original-peelle.pdf}
\caption{Original Peelle problem rewritten in the context of interferometry.  Two correlated visibility measurements $\data_1$ and $\data_2$ (points with error bars) yield an estimate $\data$ (dashed line, uncertainty in grey) outside of the data range. Systematic uncertainty from calibration is 5\% and measurement noise $\approx 0.6\%$.}
\label{fig:peelle}
\end{figure}
...
$\sigma^2$           & variance ($= \expect{\error^2}$)\\
