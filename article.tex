...
\section{Normalisation by a transfer function}

\subsection{Reframing Peelle's Pertinent Puzzle}
...
\section{Pixel-to-visibility matrix}
\def\vcovmat#1{\vec\Sigma^{\vec{#1}}}
\def\vcorrmat#1{\vec\rho^{\vec{#1}}}
\def\covmat#1{\Sigma^{\vec{#1}}}
\def\corrmat#1{\rho^{\vec{#1}}}
I call $I_p$ the luminous intensity on detector part $p$ ($1 \cdots P$).  The scalar interferometric observable $V_o$ for observable $o$ ($1 \cdots O$) is deduced using the pixel-to-visibility matrix $M_{op}$ by
\begin{align}
    \vec{V} &= \vec{M}\vec{I},
\intertext{where $\vec{M}$ and $\vec{I}$ can be written as}
    \vec{I} &= \mean{\vec{I}}(1 + \vec\error^{\vec I}),\\
    \vec{M} &= \mean{\vec{M}}(1 + \vec\error^{\vec M})
\end{align}

The covariance matrix $\covmat{V}$ can be obtained from corralation matrices 
\begin{align}
    \vcorrmat{I} &= <\vec\error^{\vec M} \outer \vec\error^{\vec I}>\\
    \vcorrmat{M} &= <\vec\error^{\vec M} \outer \vec\error^{\vec M}>
\end{align}
as
\begin{equation}
   \begin{split}
   \covmat{V}_{oo'} = \sum_{1\le p,p'\le P}&
        \bigg[\Big( 
             \corrmat{I}_{pp'}
           + \corrmat{M}_{opop'}
           - \corrmat{I}_{pp'}\corrmat{M}_{opop'}
        \Big) \times\\
        &\qquad \mean{I}_p \mean{I}_{p'} \mean{M}_{op} \mean{M}_{o'p'}\bigg], 
    \end{split}
\end{equation}
assuming that $M$ and $I$ are not correlated.

Once again the true value $\mean{\vec{I}}$ is unknown and replacing it by the
observed value $\vec{I}$ may lead to bias. Unlike in the previous section,
$\vec{I}$ is not directly produced by the modelling, so one would need
to obtain it from $\vec{V}$.  However, the pixel-to-visibility matrix is
